...
func SamplesToMatrix(samples []*sample.Sample) mat64.Matrix {
	if len(samples) < 1 {
...
func SetSampleValues(s *sample.Sample, values []float64) {
	if len(s.Values) >= len(values) {
		s.Values = s.Values[:len(values)]
	} else {
		s.Values = make([]sample.Value, len(values))
	}
	for i, val := range values {
		s.Values[i] = sample.Value(val)
	}
}
...
func (model *PCAModel) ComputeModel(samples []*sample.Sample) error {
