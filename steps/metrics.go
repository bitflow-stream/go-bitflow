...
func RegisterIncludeMetricsFilter(b builder.PipelineBuilder) {
	b.RegisterAnalysisParamsErr("include",
		...
		"Match every metric with the given regex and only include the matched metrics", builder.RequiredParams("m"))
}

func RegisterExcludeMetricsFilter(b builder.PipelineBuilder) {
	b.RegisterAnalysisParamsErr("exclude",
		...
		"Match every metric with the given regex and exclude the matched metrics", builder.RequiredParams("m"))
}
...
func RegisterMetricMapper(b builder.PipelineBuilder) {
	b.RegisterAnalysisParams("remap",
		...
		"Change (reorder) the header to the given comma-separated list of metrics", builder.RequiredParams("header"))
}
...
func RegisterVarianceMetricsFilter(b builder.PipelineBuilder) {
	b.RegisterAnalysisParamsErr("filter_variance",
		func(p *pipeline.SamplePipeline, params map[string]string) error {
			variance, err := strconv.ParseFloat(params["min"], 64)
			if err != nil {
				err = builder.ParameterError("min", err)
			} else {
		...
		builder.RequiredParams("min"), builder.SupportBatch())
}
...
func RegisterMetricRenamer(b builder.PipelineBuilder) {
	b.RegisterAnalysisParamsErr("rename",
		func(p *pipeline.SamplePipeline, params map[string]string) error {
			...
			for regex, replacement := range params {
				...
				if err != nil {
					return builder.ParameterError(regex, err)
				}
