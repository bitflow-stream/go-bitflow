...
// SampleOutput is a SampleProcessor that outputs the received samples to a
// byte stream that is generated by a Marshaller instance.
type SampleOutput interface {
	SampleProcessor
...
// AbstractSampleOutput is a partial implementation of SampleOutput
...
func (out *AbstractSampleOutput) SetMarshaller(marshaller Marshaller) {
...
func (out *AbstractSampleOutput) Sample(err error, sample *Sample, header *Header) error {
	...
	return out.OutgoingSink.Sample(sample, header)
}
...
func (s *DroppingSampleProcessor) Close() {
	s.AbstractSampleProcessor.CloseSink(nil)
}
...
func (p *NoopProcessor) CloseSink() {
	...
	p.StopChan.Stop()
	p.AbstractSampleProcessor.CloseSink(nil)
}
...
// sink. Other types that embed NoopProcessor can override this to perform
