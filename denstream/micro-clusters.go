...
func (c *BasicMicroCluster) computeRadius() float64 {
	var radius float64
	if c.w > 0 {
		cf1 := c.dist(c.cf1) / c.w
		cf2 := c.dist(c.cf2) / c.w
		diff := cf2 - cf1*cf1
...
func (c *BasicMicroCluster) computeCenter(center []float64) []float64 {
	...
	for _, cf1 := range c.cf1 {
		c.center = append(c.center, cf1/c.w)
	}
	if math.IsNaN(c.center[0]) {
		panic(fmt.Errorf("NaN in center: %v. W: %v, CF1: %v", c.center, c.w, c.cf1))
	}
	return center
}

func (c *BasicMicroCluster) dist(p []float64) float64 {
	var res float64
	for _, v := range p {
		res += v * v
	}
