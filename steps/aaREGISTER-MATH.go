...
func REGISTER_MATH(b *query.PipelineBuilder) {
...
func add_binary_evaluation(p *pipeline.SamplePipeline, params map[string]string) {
...
func add_event_evaluation(p *pipeline.SamplePipeline, params map[string]string) {
...
func add_preprocess_tags(p *pipeline.SamplePipeline, params map[string]string) error {
...
func set_cluster_tag(p *pipeline.SamplePipeline, params map[string]string) {
...
func smooth_anomalies(p *pipeline.SamplePipeline, params map[string]string) error {
...
func linear_regression(p *pipeline.SamplePipeline) {
	p.Batch(&regression.LinearRegressionBatchProcessor{})
}

func linear_regression_bruteforce(p *pipeline.SamplePipeline) {
	p.Batch(&regression.LinearRegressionBruteForce{})
}

func pca_analysis(p *pipeline.SamplePipeline, params map[string]string) error {
...
func pca_analysis_store(p *pipeline.SamplePipeline, params map[string]string) {
	p.Batch(StorePCAModel(params["file"]))
}

func pca_analysis_load(p *pipeline.SamplePipeline, params map[string]string) error {
	variance, err := parse_pca_variance(params)
	if err == nil {
...
func pca_analysis_load_stream(p *pipeline.SamplePipeline, params map[string]string) error {
	variance, err := parse_pca_variance(params)
	if err == nil {
...
func dbscan_rtree(p *pipeline.SamplePipeline) {
...
func dbscan_parallel(p *pipeline.SamplePipeline) {
	p.Batch(&dbscan.ParallelDbscanBatchClusterer{Eps: 0.3, MinPts: 5})
}

func add_denstream_rtree(p *pipeline.SamplePipeline, params map[string]string) (err error) {
	return add_denstream(p, params, false)
}

func add_denstream_linear(p *pipeline.SamplePipeline, params map[string]string) (err error) {
	return add_denstream(p, params, true)
}

func add_denstream(p *pipeline.SamplePipeline, params map[string]string, linearClusterSpace bool) (err error) {
	eps := 0.1
...
func add_sphere(p *pipeline.SamplePipeline, params map[string]string) error {
	var err error
...
func filter_convex_hull(p *pipeline.SamplePipeline) {
	p.Batch(BatchConvexHull(false))
}

func sort_convex_hull(p *pipeline.SamplePipeline) {
	p.Batch(BatchConvexHull(true))
}

func create_fft(p *pipeline.SamplePipeline) {
	p.Batch(new(BatchFft))
}

func create_rms(p *pipeline.SamplePipeline) {
