...
func TestAntlrBitflowScriptScheduleListener_ParseScript(t *testing.T) {
	scripts, errs := NewBitflowScriptScheduleParser().ParseScript("./in_data->concatenate()[cpu-tag=high]->window{asd()->bcd()}->avg()[cpu-tag=low, someOtherHint=5]->./out_data")
...
func TestAntlrBitflowScriptScheduleListener_ParseScriptWithPropagateDown(t *testing.T) {
	scripts, errs := NewBitflowScriptScheduleParser().ParseScript("./in_data->concatenate()[propagate-down=true,cpu-tag=high]->window{asd()->bcd()}->avg()->./out_data")
...
func TestAntlrBitflowScriptScheduleListener_ParseScriptWithPropagateDownAndInterrupt(t *testing.T) {
	scripts, errs := NewBitflowScriptScheduleParser().ParseScript("./in_data->concatenate()[propagate-down=true,cpu-tag=high]->window{asd()->bcd()}->avg()[newHints=sampleValue]->./out_data")
